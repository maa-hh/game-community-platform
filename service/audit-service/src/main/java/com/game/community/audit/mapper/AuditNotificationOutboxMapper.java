package com.game.community.audit.mapper;
...
public interface AuditNotificationOutboxMapper extends BaseMapper<AuditNotificationOutbox> {

    @Select("SELECT * FROM t_audit_notification_outbox WHERE status IN (0,3) "
            + "AND (next_retry_time IS NULL OR next_retry_time <= NOW()) ORDER BY id LIMIT #{limit}")
    List<AuditNotificationOutbox> selectPending(@Param("limit") int limit);

    @Update("UPDATE t_audit_notification_outbox SET status = 1, lock_token = #{token}, lock_time = NOW(), "
            + "update_time = NOW() WHERE id = #{id} AND status IN (0,3)")
    int claim(@Param("id") Long id, @Param("token") String token);

    @Update("UPDATE t_audit_notification_outbox SET status = 2, lock_token = NULL, lock_time = NULL, "
