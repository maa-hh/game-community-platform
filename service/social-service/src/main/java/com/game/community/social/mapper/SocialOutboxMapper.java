package com.game.community.social.mapper;
...
public interface SocialOutboxMapper extends BaseMapper<SocialOutboxEvent> {
    ...
    @Update("UPDATE t_social_outbox SET status = 1, lock_token = #{token}, lock_time = NOW(), "
            + "update_time = NOW() WHERE id = #{id} AND status IN (0,3)")
    int claim(@Param("id") Long id, @Param("token") String token);

    @Update("UPDATE t_social_outbox SET status = 2, lock_token = NULL, lock_time = NULL, "
    ...
    @Update("UPDATE t_social_outbox SET status = CASE WHEN retry_count + 1 >= 10 THEN 4 ELSE 3 END, "
    ...
    @Update("UPDATE t_social_outbox SET status = 3, lock_token = NULL, lock_time = NULL, "
