...
public class AuditTaskExecutor {
    ...
    public void handleRejected(Long taskId, AuditFieldType taskType, Long userId,
        ...
        if (task != null) {
            ...
            finishTaskFromAny(task.getId(), AuditTaskStatus.FAILED, "审核服务繁忙，队列已满");
        ...
        try {
            ...
            rejectLog.setRejectReason(abbreviate(cause == null ? "审核服务繁忙，队列已满" : cause.getMessage()));
    ...
    public void run(Long taskId) {
        ...
        if (payload == null || payload.getField() == null) {
            rollbackField(task.getTaskType(), task.getUserId(), payload);
            finishTaskFromAny(taskId, AuditTaskStatus.FAILED, "审核任务负载异常");
        ...
        try {
            ...
            if (result.reject()) {
                rollbackField(task.getTaskType(), task.getUserId(), payload);
                finishTaskFromAny(taskId, AuditTaskStatus.REJECTED, reason);
            ...
            if (result.humanReview()) {
                ...
                finishTaskFromAny(taskId, AuditTaskStatus.HUMAN_REVIEW, reason);
            ...
            if (!applyPassed(task.getTaskType(), task.getUserId(), payload)) {
                rollbackField(task.getTaskType(), task.getUserId(), payload);
                finishTaskFromAny(taskId, AuditTaskStatus.FAILED, "审核结果回写失败");
                notificationProducer.publishRejected(task.getUserId(), task.getTaskType(), score, "审核结果回写失败");
                return;
            }
            finishTaskFromAny(taskId, AuditTaskStatus.PASSED, UserStrings.EMPTY);
            ...
            rollbackField(task.getTaskType(), task.getUserId(), payload);
            finishTaskFromAny(taskId, AuditTaskStatus.FAILED, message);
            notificationProducer.publishRejected(task.getUserId(), task.getTaskType(),
    ...
    public void finishTaskFromAny(Long taskId, AuditTaskStatus status, String errorMessage) {
        updateTask(taskId, status, null, errorMessage);
    }
    ...
    private void updateTask(Long taskId, AuditTaskStatus status, Integer score, String errorMessage) {
