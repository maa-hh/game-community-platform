package com.game.community.steam.filter;
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (userIdStr != null && !userIdStr.isBlank()) {
            try {
                ...
                UserThreadLocal.setUser(new UserContex(
                        userId,
                        userType,
                        steamAccount == null ? "" : steamAccount,
                        sessionId));
            } catch (NumberFormatException e) {
                log.warn("Steam 服务用户上下文解析失败: userId={}, type={}", userIdStr, userTypeStr);
            }
        }
        try {
