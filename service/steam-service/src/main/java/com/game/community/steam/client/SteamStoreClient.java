...
public class SteamStoreClient {
    ...
    private List<GameAchievementVO> fetchAchievementSchema(long appId) {
        ...
        try {
            ...
            for (JsonNode item : achievements) {
                ...
                vo.setIconUrl(item.path("icon").asText(null));
                result.add(vo);
            }
            return result;
        } catch (Exception e) {
    ...
    /** 国区 Steam 榜单 appId 列表（hot/new/discount/free） */
    public List<Long> fetchChartAppIds(String board, int limit) {
        return fetchChartAppIds(board, 0, limit);
    }

    /**
     * 按榜单偏移量拉取 Steam 候选 ID。首屏优先使用精选榜单，后续页使用 Steam 搜索结果的
     * start/count 做按需扩展，避免服务启动时一次性拉取大量游戏详情。
     */
    public List<Long> fetchChartAppIds(String board, int start, int limit) {
        ...
        if (safeStart > 0) {
            return fetchSearchChartAppIds(board, safeStart, capped);
        }
        List<Long> appIds = new ArrayList<>();
        if (!GameBoardConstants.FREE.equalsIgnoreCase(board)) {
            String section = resolveFeaturedSection(board);
            if (StringUtils.hasText(section)) {
                appIds.addAll(extractFeaturedAppIds(loadFeaturedCategoriesRoot(), section, capped));
            }
        }
        if (appIds.size() < capped) {
            appIds = mergeAppIds(appIds, fetchSearchChartAppIds(board, 0, capped));
        }
        if (appIds.size() > capped) {
            return new ArrayList<>(appIds.subList(0, capped));
        }
        return appIds;
    }

    private List<Long> mergeAppIds(List<Long> primary, List<Long> extra) {
    ...
    private List<Long> extractFeaturedAppIds(JsonNode root, String section, int limit) {
        List<Long> appIds = new ArrayList<>();
    ...
    private List<Long> fetchSearchChartAppIds(String board, int start, int limit) {
    ...
    private List<Long> fetchFreeChartAppIds(int limit) {
        return fetchSearchChartAppIds(GameBoardConstants.FREE, 0, limit);
    }
}
