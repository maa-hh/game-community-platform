...
运维：已有库执行 `sql/content-posting-alter.sql`；content-service multipart 已调至 6MB/片。
...
| 审核通过上架 | **公共桶** | 复制/提升后改写正文与封面 URL |
| 驳回 / 取消 / 删除 | — | 删除私有对象与分片残留 |
...
3. 审核通过：`private → public`，正文 URL 替换为公共地址  
4. 审核读图：走服务端从私有桶读流，不依赖公网 URL  
...
编辑中(DRAFT)
  ...
  → 提交审核(PENDING, 任务入队)
  → 审核中(进度:文本/图片/视频)
  → 通过(PUBLISHED, 媒体转公有) | 驳回(REJECTED, 清私有可选保留草稿)
任意未发布阶段可「取消/删除」→ 停任务 + 清 MinIO 残留
```
